...
if(isset($_POST['search'])){
    ...
    $query = "SELECT * FROM bank_details_users WHERE bank_id LIKE '%$q%' OR bank_name LIKE '%$q%'";

    $stm = $conn->prepare($query);
    $stm->execute();
    $result_userbank = $stm->fetchAll();

    if($result_userbank > 0){
        $response = "<ul>";
        foreach ($result_userbank as $result){
            $response .= "<li>". "ID: ". $result['bank_id']."| ".$result['bank_name']."</li>";
        }
        $reponse .= "</ul>";
    }
    $stm->closeCursor();
...
<html>
    ...
    <body style="color: white;font-weight:700">
            <!-- search box -->
            <input type="search" placeholder="SEARCH USER BANK HERE" id="search_box">
            <div id="response"></div>
