...
if(isset($_POST['search'])){
    ...
    $query = "SELECT * FROM partner_organizations WHERE name LIKE '%$q%' OR location LIKE '%$q%'";

    $stm = $conn->prepare($query);
    $stm->execute();
    $result_partners = $stm->fetchAll();

    if($result_partners > 0){
        $response = "<ul>";
        foreach ($result_partners as $result){
            $response .= "<li>".$result['name']." ".$result['location']."</li>";
        }
        $reponse .= "</ul>";
    }
    $stm->closeCursor();
...
<html>
    ...
    <body style="color: white;font-weight:700">
            <!-- search box -->
            <input type="search" placeholder="SEARCH ORGANIZATIONS HERE" id="search_box">
            <div id="response"></div>
